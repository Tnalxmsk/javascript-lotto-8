...
describe("당첨 로또 클래스 테스트", () => {
  test("내 로또 번화 당첨 로또의 매칭 개수를 반환한다.", () => {
    ...
    winningLotto.registerBonusNumber(10);
  ...
  test("보너스 넘버를 가지고 있다면 true를 반환한다.", () => {
    const myLotto = new Lotto([1, 2, 3, 4, 5, 10]);
    const winningLotto = new WinningLotto([1, 2, 3, 4, 5, 8]);
    winningLotto.registerBonusNumber(10);
  ...
  test("보너스 번호 값이 문자라면 예외가 발생한다.", () => {
    expect(() => {
      const winningLotto = new WinningLotto([1, 2, 3, 4, 5, 8]);
      winningLotto.registerBonusNumber("a");
    }).toThrow("[ERROR]");
  });
  ...
  test("보너스 번호가 당첨번호와 중복이면 예외가 발생한다.", () => {
    expect(() => {
      const winningLotto = new WinningLotto([1, 2, 3, 4, 5, 8]);
      winningLotto.registerBonusNumber(1);
    }).toThrow("[ERROR]");
  });
  ...
  test("보너스 번호가 1부터 45사이의 숫자 값이 아니라면 예외가 발생한다.", () => {
    expect(() => {
      const winningLotto = new WinningLotto([1, 2, 3, 4, 5, 8]);
      winningLotto.registerBonusNumber(46);
